...
export interface Report {
  id: string;
  type: 'ways_count' | 'active_ways' | 'overdue_ways' | 'ways_by_town' | 'ways_by_deliveryman' | 'ways_by_merchant' | 'merchant_compare';
  title: string;
  periodStart: string;
  periodEnd: string;
...
export function getStatusColor(status: DeliveryStatus | PaymentStatus): string {
  const statusColorMap: Record<string, string> = {
    ...
    [DeliveryStatus.DELIVERED]: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
    [DeliveryStatus.FAILED]: 'bg-destructive/10 text-destructive',
    [DeliveryStatus.RETURNED]: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    [DeliveryStatus.CANCELLED]: 'bg-muted text-muted-foreground',
    [`payment_${PaymentStatus.PENDING}`]: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    [`payment_${PaymentStatus.PAID}`]: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
