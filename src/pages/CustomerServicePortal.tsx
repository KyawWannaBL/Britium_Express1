...
const merchantMaster: Merchant[] = [
  { id: "MRC-001", name: "Baby Kyaw", code: "BBK", contact: "Merchant Contact", phone: "Registered phone", address: "Default pickup address, South Okkalapa", township: "South Okkalapa", city: "Yangon", pickupTime: "10:00 AM - 12:00 PM", payment: "COD" },
  { id: "MRC-002", name: "Baby World", code: "BBW", contact: "Merchant Contact", phone: "Registered phone", address: "Default pickup address, East Dagon", township: "East Dagon", city: "Yangon", pickupTime: "01:00 PM - 03:00 PM", payment: "Account" },
  { id: "MRC-003", name: "City Fresh", code: "CTF", contact: "Merchant Contact", phone: "Registered phone", address: "Default pickup address, Hlaing", township: "Hlaing", city: "Yangon", pickupTime: "02:00 PM - 04:00 PM", payment: "Prepaid" },
];
...
function createIds(date: string, code: string, parcels: number) {
  const cleanCode = code.toUpperCase().slice(0, 3).padEnd(3, "X");
  return {
    pickupId: `P${mmdd(date)}-${cleanCode}-${pad3(parcels)}`,
    deliverId: `D${mmdd(date)}-${cleanCode}-${pad3(parcels + 1)}`,
    invoiceNo: `I${mmdd(date)}-${cleanCode}-${pad3(parcels)}`,
    waybillNo: `W${mmdd(date)}-${cleanCode}-${pad3(parcels)}`,
  };
}

const initialPickups: PickupRequest[] = [
  { ...createIds("2026-05-25", "BBW", 15), merchantName: "Baby World", merchantCode: "BBW", merchantAddress: "Default pickup address, East Dagon", pickupDate: "2026-05-25", pickupTime: "01:00 PM - 03:00 PM", parcelCount: 15, payment: "Account", priority: "Medium", status: "DATA ENTRY IN PROGRESS" },
  { ...createIds("2026-05-25", "BBK", 15), merchantName: "Baby Kyaw", merchantCode: "BBK", merchantAddress: "Default pickup address, South Okkalapa", pickupDate: "2026-05-25", pickupTime: "10:00 AM - 12:00 PM", parcelCount: 15, payment: "COD", priority: "High", status: "READY FOR DATA ENTRY" },
];
...
export default function CustomerServicePortalPage() {
  ...
  const [notice, setNotice] = useState("Customer Service queue is ready. Canonical pickup queue and merchant master are loaded.");

  const merchant = useMemo(() => merchantMaster.find((row) => row.id === merchantId) ?? merchantMaster[0], [merchantId]);
  const count = Math.max(1, Number(parcelCount) || 1);
  const ids = useMemo(() => createIds(pickupDate, merchant.code, count), [count, merchant.code, pickupDate]);
  ...
  function createPickup() {
    const next: PickupRequest = {
      ...
      pickupDate,
      pickupTime: merchant.pickupTime,
      parcelCount: count,
    ...
    setNotice(`${next.pickupId} created for Data Entry. ${next.deliverId}, ${next.invoiceNo}, and ${next.waybillNo} were auto-filled.`);
  }
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fc] p-8 text-slate-950">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={() => setNotice("Customer Service queue refreshed.")} className="inline-flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-7 py-4 text-lg font-black shadow-sm hover:bg-slate-50"><RefreshCw size={20} />Refresh</button>
      </div>
      ...
      <div className="mt-8 rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 xl:grid-cols-[1fr_180px]">
          ...
          <button type="button" onClick={() => setNotice("Search synced with canonical pickup records.")} className="rounded-2xl bg-blue-700 px-6 py-4 text-lg font-black text-white shadow-sm hover:bg-blue-800">Search / Sync</button>
        </div>
